...
namespace heat {
    ...
    void HeatEquationSolver1D::initializeMatrix() {
        temperatureMatrix = new double*[N_t];
        for (int t = 0; t < N_t; ++t) {
            temperatureMatrix[t] = new double[N_s];
            for (int x = 0; x < N_s; ++x) {
    ...
    void HeatEquationSolver1D::applyDirichletBoundary(double* currentRow) {
        currentRow[N_s - 1] = u0; /** Dirichlet boundary condition (fixed temperature) */
    }
    ...
    void HeatEquationSolver1D::solve() {
        ...
        double* a = new double[N_s - 1]; /** Sub-diagonal */
        double* b = new double[N_s];     /** Main diagonal */
        double* c = new double[N_s - 1]; /** Super-diagonal */
        double* d = new double[N_s];     /** Right-hand side */
        ...
        for (int n = 0; n < N_t - 1; ++n) {
            ...
            d[N_s - 1] = u0;                      /** Dirichlet at x = L */

            /** Solve the tridiagonal system */
            solveTridiagonal(a, b, c, d, N_s);
